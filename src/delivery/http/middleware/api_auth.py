...
class ApiAuthMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            await self.rate_limiter.record_request(
                api_key=api_key, client_ip=client_ip, endpoint=request.url.path
            )
    ...
    async def _authenticate_with_api_key(self, api_key_str: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": True, "api_key": api_key, "auth_method": "api_key"}
    ...
    def _create_error_response(
        self, status_code: int, error: str, details: str = None
    ) -> JSONResponse:
        """Создает стандартизированный ответ об ошибке"""
        content = {"error": error, "timestamp": time.time(), "status_code": status_code}
        ...
        return JSONResponse(
            ...
            content=content,
            headers={"X-Error-Type": "authentication_error", "X-Error-Code": str(status_code)},
        )
...
def create_api_auth_middleware(
    jwt_service: JWTService, rate_limiter: RateLimiter, api_key_repo, config: Dict[str, Any] = None
):
    """
    ...
    protected_paths = config.get(
        "protected_paths",
        ["/api/v1/detect", "/api/v1/detect/batch", "/api/v1/stats", "/api/v1/account"],
    )
