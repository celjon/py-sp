...
class CreateApiKeyRequestV2(BaseModel):
    ...
    client_name: str = Field(
        ..., min_length=2, max_length=100, description="Название клиента/проекта"
    )
    ...
    requests_per_month: Optional[int] = Field(
        None, ge=1, le=5000000, description="Запросов в месяц"
    )
...
async def get_jwt_tokens(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to generate tokens"
        )
...
@router.post("/revoke", summary="Отозвать токен", description="Отзывает access или refresh токен")
async def revoke_token(
    ...
    try:
        ...
        if not success:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid token or already revoked"
            )
    ...
    try:
        ...
        for api_key in api_keys:
            ...
            if include_usage_stats:
                try:
                    usage_stats_24h = await usage_analytics.get_real_time_metrics(
                        api_key.id, 24 * 60
                    )
                except Exception as e:
...
async def get_key_analytics(
    ...
    try:
        ...
        metrics = await usage_analytics.get_usage_metrics(
            api_key_id=key_id, period=period, hours_back=hours_back
        )

        # Детектируем аномалии
        anomalies = await usage_analytics.detect_anomalies(api_key_id=key_id, hours_back=hours_back)
...
async def auth_health_check(
    ...
    try:
        ...
        if jwt_health.get("status") != "healthy" or analytics_health.get("status") != "healthy":
            overall_status = "degraded"

        return {
            "status": overall_status,
            "timestamp": time.time(),
            "components": {"jwt_service": jwt_health, "usage_analytics": analytics_health},
            "version": "2.0.0",
        }
