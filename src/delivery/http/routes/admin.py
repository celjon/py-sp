...
def verify_admin_credentials(
    credentials: HTTPBasicCredentials = Depends(security), config=Depends(get_config)
):
...
@router.get(
    "/status", response_model=SystemStatus, dependencies=[Depends(verify_admin_credentials)]
)
async def get_system_status(dependencies: Dict[str, Any] = Depends(get_dependencies)):
    ...
    try:
        ...
        if (
            database_status.get("status") == "error"
            or redis_status.get("status") == "error"
            or telegram_status.get("status") == "error"
        ):
...
@router.get(
    "/samples", response_model=SpamSamplesList, dependencies=[Depends(verify_admin_credentials)]
)
async def get_spam_samples(
    ...
    try:
        ...
        if not spam_samples_repo:
            raise HTTPException(status_code=503, detail="Spam samples repository not available")

        # Получаем образцы по типу
        if sample_type:
            if sample_type not in ["spam", "ham"]:
                raise HTTPException(
                    status_code=400, detail="Invalid sample_type. Must be 'spam' or 'ham'"
                )
            ...
            samples = await spam_samples_repo.get_recent_samples(
                hours=24 * 30, limit=per_page
            )  # За месяц
        ...
        return SpamSamplesList(
            samples=sample_responses, total_count=total_count, page=page, per_page=per_page
        )

    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Error getting spam samples: {str(e)}")


@router.post(
    "/samples", response_model=SpamSampleResponse, dependencies=[Depends(verify_admin_credentials)]
)
async def create_spam_sample(
    sample_data: SpamSampleCreate, dependencies: Dict[str, Any] = Depends(get_dependencies)
):
    ...
    try:
        ...
        if sample_data.type not in ["spam", "ham"]:
            raise HTTPException(status_code=400, detail="Invalid type. Must be 'spam' or 'ham'")
        ...
        raise HTTPException(status_code=500, detail=f"Error creating spam sample: {str(e)}")


@router.delete("/samples/{sample_id}", dependencies=[Depends(verify_admin_credentials)])
async def delete_spam_sample(
    sample_id: int, dependencies: Dict[str, Any] = Depends(get_dependencies)
):
    ...
    try:
        ...
        if not success:
            raise HTTPException(status_code=404, detail="Spam sample not found")
        ...
        raise HTTPException(status_code=500, detail=f"Error deleting spam sample: {str(e)}")


@router.get("/detectors/config", dependencies=[Depends(verify_admin_credentials)])
async def get_detectors_config(dependencies: Dict[str, Any] = Depends(get_dependencies)):
    ...
    try:
        ...
        # Современная архитектура: только CAS + RUSpam + OpenAI
        ...
        raise HTTPException(status_code=500, detail=f"Error getting detectors config: {str(e)}")
...
async def test_detectors(
    ...
    try:
        ...
        if not check_message_usecase:
            raise HTTPException(status_code=503, detail="Check message use case not available")

        from ....domain.entity.message import Message

        # Создаем тестовое сообщение
        message = Message(user_id=0, chat_id=0, text=test_text)
        ...
        raise HTTPException(status_code=500, detail=f"Error testing detectors: {str(e)}")
