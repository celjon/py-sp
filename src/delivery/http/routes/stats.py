...
async def get_global_stats(
    ...
    try:
        ...
        if not message_repo:
            raise HTTPException(
                status_code=503,
                detail="Message repository not available"
            )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error getting global stats: {str(e)}"
        )
...
async def get_user_stats(
    ...
    try:
        ...
        stats = await message_repo.get_user_stats(
            user_id=user_id, 
            chat_id=chat_id, 
            hours=hours
        )
    ...
    try:
        ...
        if spam_detector:
            ...
            for detector_name in available_detectors:
                detector_performance.append(DetectorPerformance(
...
async def get_top_spammers(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error getting top spammers: {str(e)}"
        )
...
async def get_detector_stats(
    ...
    try:
        ...
        if detector_name == "heuristic" and hasattr(spam_detector, 'heuristic_detector'):
            result["config"] = spam_detector.heuristic_detector.get_config()
            result["statistics"] = spam_detector.heuristic_detector.get_statistics()
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error getting detector stats: {str(e)}"
        )
