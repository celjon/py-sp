...
class DetectSpamUseCase:
    ...
    async def execute(
        ...
        try:
            ...
            response = DetectionResponse(
                ...
                reason=detection_result.primary_reason.value if detection_result.primary_reason else "unknown",
                action=action,
            ...
            await self._record_usage(
                api_key, request, response, RequestStatus.SUCCESS
            )
            ...
            await self._record_usage(
                api_key, request, error_response, RequestStatus.ERROR
            )
    ...
    def _get_detected_patterns(self, detection_result: DetectionResult) -> Optional[list]:
        """Извлекает обнаруженные паттерны спама"""
        patterns = []
        
        for detector_result in detection_result.detector_results:
            if detector_result.is_spam and detector_result.detector_name == "heuristic":
                # Парсим детали эвристического детектора
                details = detector_result.details.lower()
                
                if "emoji" in details:
                    patterns.append("excessive_emojis")
                if "caps" in details:
                    patterns.append("excessive_caps")
                if "links" in details:
                    patterns.append("suspicious_links")
                if "mentions" in details:
                    patterns.append("excessive_mentions")
                if "repeated" in details:
                    patterns.append("repeated_characters")
                if "spam patterns" in details:
                    patterns.append("spam_phrases")
            
            elif detector_result.is_spam and detector_result.detector_name == "cas":
                patterns.append("cas_banned_user")
            
            elif detector_result.is_spam and detector_result.detector_name in ["RUSpam", "ML_Classifier"]:
                patterns.append("ml_classifier")
