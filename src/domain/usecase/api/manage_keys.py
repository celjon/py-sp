...
class ManageApiKeysUseCase:
    ...
    def __init__(
        self, api_key_repo: ApiKeyRepository, usage_repo: Optional[UsageRepository] = None
    ):
    ...
    async def create_api_key(self, request: CreateApiKeyRequest) -> ApiKeyResponse:
        ...
        if request.requests_per_hour is not None:
            api_key.requests_per_hour = request.requests_per_hour
    ...
    async def suspend_api_key(
        self, api_key_id: int, reason: str = None
    ) -> Optional[ApiKeyResponse]:
    ...
    async def get_api_key_usage_stats(
        self, api_key_id: int, period: UsagePeriod = UsagePeriod.DAY, days: int = 30
        ...
        usage_stats = await self.usage_repo.get_usage_stats(
            api_key_id=api_key_id, period=period, start_time=start_time
        )
    ...
    async def get_global_statistics(self) -> Dict[str, Any]:
        ...
        return {"api_keys": stats, "timestamp": datetime.now(timezone.utc).isoformat()}
