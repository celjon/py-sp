...
class RateLimitInfo:
    ...
    def to_headers(self) -> Dict[str, str]:
        ...
        return {
            ...
            "X-RateLimit-Remaining-Minute": str(max(0, self.requests_per_minute - self.current_minute)),
...
class RateLimiter:
    ...
    def __init__(self, redis_client=None, fallback_mode: bool = True):
        ...
        print(f"🚥 Rate Limiter инициализирован (Redis: {'✅' if redis_client else '❌'}, fallback: {fallback_mode})")
    ...
    async def get_rate_limit_info(self, api_key: ApiKey) -> RateLimitInfo:
        ...
        try:
            ...
            current_minute = await self._get_window_count(api_key.id, RateLimitType.PER_MINUTE, current_time, 60)
            current_hour = await self._get_window_count(api_key.id, RateLimitType.PER_HOUR, current_time, 3600)
            current_day = await self._get_window_count(api_key.id, RateLimitType.PER_DAY, current_time, 86400)
    ...
    async def _check_window_limit(
        ...
        current_time = int(time.time())
        current_count = await self._get_window_count(api_key_id, limit_type, current_time, window_seconds)
