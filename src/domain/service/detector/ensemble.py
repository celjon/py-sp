...
class EnsembleDetector:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        logger.info(f"   Пороги: spam={self.spam_threshold}, high={self.high_confidence_threshold}, auto_ban={self.auto_ban_threshold}")
    ...
    def _record_detector_failure(self, detector_name: str, error: Exception):
        ...
        if breaker:
            ...
            if breaker.failure_count >= self.circuit_breaker_threshold:
                ...
                logger.warning(f"🚨 Circuit breaker для {detector_name} открыт после {breaker.failure_count} ошибок")
    ...
    async def detect(self, message: Message, user_context: Dict[str, Any] = None) -> DetectionResult:
        ...
        logger.info(f"🔍 Детекция #{self._detection_count}: '{text[:50]}{'...' if len(text) > 50 else ''}' (язык: {detected_language})")
        ...
        try:
            ...
            if cas_result:
                ...
                if cas_result.is_spam:
                    ...
                    logger.warning(f"🚨 CAS BAN: пользователь {message.user_id} ({final_result.processing_time_ms:.1f}ms)")
                    return final_result
            ...
            if ruspam_result:
                ...
                if ruspam_result.is_spam:
                    ...
                    if (self.enable_early_exit and 
                        ...
                        logger.warning(f"🚨 EARLY EXIT: RUSpam уверенность {ruspam_result.confidence:.3f} ({final_result.processing_time_ms:.1f}ms)")
                        return final_result
            ...
            if elapsed_time >= self.max_processing_time:
                logger.warning(f"⏰ Превышен лимит времени ({elapsed_time:.2f}s), пропускаем OpenAI")
            else:
            ...
            logger.info(f"{result_emoji} Детекция завершена: spam={is_spam_detected}, confidence={max_confidence:.3f}, время={final_result.processing_time_ms:.1f}ms")
    ...
    async def _check_cas(self, message: Message, user_context: Dict[str, Any]) -> Optional[DetectorResult]:
        ...
        try:
            ...
            if cas_result.is_spam:
                logger.warning(f"🚨 CAS: Пользователь {message.user_id} забанен ({processing_time:.1f}ms)")
            else:
    ...
    async def _check_ruspam(self, text: str, language: str) -> Optional[DetectorResult]:
        ...
        try:
            ...
            if ruspam_result.is_spam:
                logger.warning(f"🚨 RUSpam: СПАМ обнаружен ({ruspam_result.confidence:.3f}, {processing_time:.1f}ms)")
            else:
                logger.debug(f"✅ RUSpam: Сообщение чистое ({1.0 - ruspam_result.confidence:.3f}, {processing_time:.1f}ms)")
    ...
    async def _check_openai(self, message: Message, user_context: Dict[str, Any], text: str) -> Optional[DetectorResult]:
        ...
        try:
            ...
            if openai_result.is_spam:
                logger.warning(f"🚨 OpenAI: СПАМ обнаружен ({openai_result.confidence:.3f}, {processing_time:.1f}ms)")
            else:
    ...
    def _determine_action(self, confidence: float, is_spam: bool, user_context: Dict[str, Any] = None) -> str:
        """
    ...
    def _update_performance_metrics(self, processing_time_ms: float):
        ...
        if processing_time_ms > self.max_processing_time * 1000:
            logger.warning(f"⚠️ Медленная детекция: {processing_time_ms:.1f}ms (лимит: {self.max_processing_time * 1000}ms)")
    ...
    async def get_performance_stats(self) -> Dict[str, Any]:
        ...
        avg_processing_time = (
            self._total_processing_time / self._detection_count 
            if self._detection_count > 0 else 0
        )
        
        error_rate = (
            self._error_count / self._detection_count 
            if self._detection_count > 0 else 0
        )
