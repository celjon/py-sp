...
class OpenAIDetector:
    ...
    async def detect(self, message: Message, user_context: Dict[str, Any] = None) -> DetectorResult:
        ...
        try:
            ...
            # Парсим результат
            is_spam, confidence, reasoning, token_usage = self._parse_openai_result(openai_result)

            if is_spam:
                self._spam_detected += 1
                logger.warning(
                    f"🚨 OpenAI: СПАМ обнаружен (confidence: {confidence:.3f}, время: {processing_time_ms:.1f}ms)"
                )
                logger.warning(f"   Обоснование: {reasoning}")
            else:
                logger.debug(
                    f"✅ OpenAI: сообщение чистое (confidence: {confidence:.3f}, время: {processing_time_ms:.1f}ms)"
                )

            # Логируем использование токенов
            if token_usage:
                logger.debug(
                    f"   Токены: {token_usage['input_tokens']} input + {token_usage['output_tokens']} output = {token_usage['total_tokens']} total"
                )
    ...
    async def _analyze_with_retry(self, text: str, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                result = await asyncio.wait_for(
                    self.openai_gateway.analyze_spam(text, context), timeout=self.timeout
                )
        ...
        raise RuntimeError(
            f"OpenAI analysis failed after {self.max_retries + 1} attempts: {last_error}"
        )

    def _parse_openai_result(
        self, openai_result: Dict[str, Any]
    ) -> tuple[bool, float, str, Optional[Dict[str, int]]]:
        """
        Парсит результат OpenAI и извлекает is_spam, confidence, reasoning, token_usage

        Returns:
            (is_spam, confidence, reasoning, token_usage)
        """
        try:
            ...
            return is_spam, confidence, reasoning, token_usage
            ...
            # Fallback parsing
            return False, 0.0, "Failed to parse OpenAI result", None
    ...
    async def batch_detect(
        self, messages: List[Message], user_contexts: List[Dict[str, Any]] = None
    ) -> List[DetectorResult]:
        """
            ...
            try:
                ...
                if hasattr(self.openai_gateway, "analyze_spam_batch"):
                    batch_results = await self.openai_gateway.analyze_spam_batch(
                        [msg.text for msg in msg_group], language=language
                    )

                    for i, msg in enumerate(msg_group):
                        if i < len(batch_results):
                            is_spam, confidence, reasoning, token_usage = self._parse_openai_result(
                                batch_results[i]
                            )
                            results.append(
                                DetectorResult(
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        spam_detection_rate = (
            self._spam_detected / self._successful_requests if self._successful_requests > 0 else 0
        )
    ...
    async def validate_configuration(self) -> Dict[str, Any]:
        ...
        Returns:
            Результат валидации
        """
        validation_result = {"status": "valid", "errors": [], "warnings": [], "recommendations": []}
        ...
        try:
            ...
            if self.timeout < 1.0:
                validation_result["warnings"].append(
                    "Very low timeout - may cause frequent failures"
                )
            ...
            if self.min_text_length < 5:
                validation_result["recommendations"].append(
                    "Consider increasing min_text_length for better accuracy"
                )

            if self.max_text_length > 8000:
                validation_result["warnings"].append(
                    "High max_text_length may increase costs and latency"
                )
