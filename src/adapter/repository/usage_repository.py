...
class UsageRepository:
    ...
    async def get_rate_limit_status(self, api_key_id: int) -> RateLimitStatus:
        ...
        async with self.db.acquire() as conn:
            row = await conn.fetchrow(
                query, api_key_id, minute_start, hour_start, day_start, month_start, now
            )
    ...
    async def get_hourly_usage_stats(self, api_key_id: int, days: int = 7) -> List[Dict[str, Any]]:
    ...
    async def get_top_endpoints(
        self, api_key_id: int, hours: int = 24, limit: int = 10
    ) -> List[Dict[str, Any]]:
        ...
        async with self.db.acquire() as conn:
            rows = await conn.fetch(
                query, api_key_id, RequestStatus.SUCCESS.value, start_time, limit
            )
        ...
        async with self.db.acquire() as conn:
            ...
            return {
                ...
                "spam_detection_rate": round(
                    (
                        (
                            (row["spam_detected"] or 0)
                            / ((row["spam_detected"] or 0) + (row["clean_detected"] or 0))
                        )
                        if (row["spam_detected"] or 0) + (row["clean_detected"] or 0) > 0
                        else 0.0
                    ),
                    3,
                ),
                ...
                "total_data_processed_mb": round(
                    (row["total_data_processed"] or 0) / 1024 / 1024, 2
                ),
                "period_hours": hours,
    ...
    async def get_usage_by_client_ip(
        self, api_key_id: int, hours: int = 24, limit: int = 20
    ) -> List[Dict[str, Any]]:
                ...
                result.append(
                    {
                        ...
                        "last_request_time": (
                            row["last_request_time"].isoformat()
