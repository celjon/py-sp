...
class PostgresClient:
    ...
    async def acquire(self):
        ...
        try:
            connection = await asyncio.wait_for(self.pool.acquire(), timeout=self.query_timeout)
        ...
        try:
            async with self.acquire() as conn:
                result = await asyncio.wait_for(
                    conn.execute(query, *args), timeout=timeout or self.query_timeout
                )
    ...
    async def fetchrow(self, query: str, *args, timeout: float = None) -> Optional[asyncpg.Record]:
        ...
        try:
            async with self.acquire() as conn:
                result = await asyncio.wait_for(
                    conn.fetchrow(query, *args), timeout=timeout or self.query_timeout
                )
    ...
    async def fetch(self, query: str, *args, timeout: float = None) -> List[asyncpg.Record]:
        ...
        try:
            async with self.acquire() as conn:
                result = await asyncio.wait_for(
                    conn.fetch(query, *args), timeout=timeout or self.query_timeout
                )
    ...
    async def fetchval(self, query: str, *args, timeout: float = None) -> Any:
        ...
        try:
            async with self.acquire() as conn:
                result = await asyncio.wait_for(
                    conn.fetchval(query, *args), timeout=timeout or self.query_timeout
                )
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        avg_query_time = (
            self._total_query_time / self._total_queries if self._total_queries > 0 else 0
        )

        error_rate = self._failed_queries / self._total_queries if self._total_queries > 0 else 0

        connection_error_rate = (
            self._connection_errors / self._connections_created
            if self._connections_created > 0
            else 0
        )
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        """
        health_info = {"status": "unknown", "timestamp": time.time(), "database": "postgresql"}

        try:
            if not self.pool:
                health_info.update(
                    {"status": "disconnected", "error": "Connection pool not initialized"}
                )
                return health_info
